...
`Parada_aux(codigo_ruta, num_secuencia, nombre_parada)`$\supseteq$
...
`Parada(ruta, estacion, num_secuencia, km_origen)` $\supseteq$
...
`Ruta(id, origen, destino, tipo)` $\supseteq$
...
`Viaje(id, ruta, fecha, horario)` $\supseteq$
